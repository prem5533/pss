...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 LOOX irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on safepecentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in safepe.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("safepe.conf", statusObj));
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by masternode configured in safepe.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* SafePe features */
    { "safepe",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "safepe",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "safepe",               "gobject",                &gobject,                true,  {} },
    { "safepe",               "voteraw",                &voteraw,                true,  {} },
