...
const char * const BITCOIN_CONF_FILENAME = "safepe.conf";
const char * const BITCOIN_PID_FILENAME = "safeped.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "safepe" is a composite category enabling all SafePe-related debug output
                if(ptrCategory->count(std::string("safepe"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SafePe
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SafePe
    // Mac: ~/Library/Application Support/SafePe
    // Unix: ~/.safepe
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SafePe";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SafePe";
#else
    // Unix
    return pathRet / ".safepe";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty safepe.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override safepe.conf
