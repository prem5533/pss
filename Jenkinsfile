...
// which are part of the SafePe repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="safepe-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /safepe-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "safepepay-safepe/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from safepepay-safepe/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'safepepay-safepe/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from safepepay-safepe/develop"
            }
          }

          if (hasCache) {
            sh "cd /safepe-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /safepe-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /safepe-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /safepe-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /safepe-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /safepe-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /safepe-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
